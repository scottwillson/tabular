...
module Tabular
  class ColumnsTest < Minitest::Test
    def test_new_blank
      ...
      assert_equal false, columns.key?(:name), "has_key? :name"
    ...
    def test_new
      ...
      assert_equal false, columns.key?(:location), "has_key? :location"
      assert_equal true, columns.key?(:date), "has_key? :date"
      assert_equal true, columns.key?(:first_name), "has_key? :first_name"
      assert_equal true, columns.key?(:last_name), "has_key? :last_name"
      assert_equal false, columns.key?("first name"), "has_key? 'first name'"
    ...
    def test_delete
      ...
      assert_equal %i[first_name last_name], columns_from_each, "column keys from #each"

      assert_equal false, columns.key?(:date), "has_key? :date"
      assert_equal true, columns.key?(:first_name), "has_key? :first_name"
    ...
    def test_push_onto_blank
      columns = Columns.new(nil, [])
      columns << "city state"
      assert_equal true, columns.key?(:city_state), "has_key? :city_state"
    ...
    def test_push
      columns = Columns.new(nil, %w[first second])
      columns << "third"
      assert_equal true, columns.key?(:third), "has_key? :third"
